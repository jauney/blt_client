...
export function getPrintOrderConent({
  ...
  let html = `
    ...
    <table>
      <tr>
        <td class="col2-1">到货站:${data.company_name || ''}</td>
        <td class="col2-2 txt-bold">单号:${data.order_code || ''}</td>
      </tr>
      <tr>
        <td style="font-size: 11px" colspan="2">日期:${orderDate}</td>
      </tr>
    </table>
    <table>
      <tr>
        <td style="width:10%">${getCustomerType}</td>
        <td style="width:90%;" class="txt-bold">收货:${data.getcustomer_name ||
    ...
    <table>
      <tr>
        <td style="font-size: 11px;">收货地址:${data.getcustomer_address ||
          getCustomer.customer_address ||
          ''}</td>
      </tr>
      <tr>
        <td class="txt-bold">备注:${data.remark || ''}</td>
      </tr>
    </table>
    <table>
      <tr>
    ...
    <table>
      <tr>
        <td colspan="2" style="font-size:12px">扫码查看公司信息、货款查询</td>
      </tr>
      <tr class="txt-desc">
        <td class="col2-1">
        ...
        <td class="col2-2">
          <div>运输协议:1.货物按国家安全标准,不予托运危险品、国家禁运品，隐瞒托运此类物品发生一切后果由托运人负责。2.所运物品自愿投保,如未投保,按照所丢失物品运费的3—5倍赔付；如有保险按投保价赔付。3.一切货损在总价值的3%以内属正常,本公司不予赔付。4.易碎物品,本公司只负责丢失赔偿，不负责损坏赔偿。5.本公司只负责货物外包装完好与发货数量，不负责货物包装内的数量与质量。6.发货人阅读托运协议,确保发货无误,发货开单即默认签署同意托运协议。</div>
        </td>
      </tr>
    </table>
    </div>
