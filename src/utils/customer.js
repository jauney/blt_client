...
export async function onCourierSelect(context, value, type) {
  const {
    courier: { receiverList = [], senderList = [] },
  } = context.props;
  let courier;
...
export async function setCustomerFieldValue(context, fieldsValue = {}, type = 'search') {
  ...
  console.log(currentSendCustomer, currentGetCustomer);
  // 编辑时的客户，使用list中的数据，因为没有currentSendCustomer，currentGetCustomer
  sendCustomerList.forEach(item => {
    if (
      item.customer_id == fieldsValue.sendcustomer_name ||
      item.customer_id == fieldsValue.sendcustomer_id
    ) {
      sendCustomer = item;
    }
  });
  getCustomerList.forEach(item => {
    if (
      item.customer_id == fieldsValue.getcustomer_name ||
      item.customer_id == fieldsValue.getcustomer_id
    ) {
      getCustomer = item;
    }
  });
...
export async function setCustomerFieldValue2Mng(context, fieldsValue = {}, type = 'search') {
  ...
  sendCustomerList.forEach(item => {
    if (
      item.customer_id == fieldsValue.customer_name ||
      item.customer_id == fieldsValue.customer_id
    ) {
      sendCustomer = item;
    }
  });
